		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
                    ...
                    html += '					<div class="explorer_btn"><a href="/upload/explorer" target="_blank">资源管理器</a></div>';
			...
			this.funDisposePreviewHtml = function(file, e){
                ...
                }else if(file.name.indexOf("ppt") > 0){
                    fileImgSrc = fileImgSrc + "ppt.png";
                }else if(file.name.indexOf("psd") > 0){
                    fileImgSrc = fileImgSrc + "psd.png";
                }else if(file.name.indexOf("swf") > 0){
                    fileImgSrc = fileImgSrc + "swf.png";
                }else if(file.name.indexOf("ttf") > 0){
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						var msgO = JSON.parse(response);
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#uploadInf").append("<p>上传成功，文件地址是：" + msgO.msg + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
